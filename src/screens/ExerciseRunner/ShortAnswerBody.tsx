...
/**
 * `short_answer` body. Free-text open comprehension answer. Unlike the
 * translate templates, this one CAN auto-score (exact match against
 * `accepted_answers`) but otherwise routes to review rather than ever
 * showing a flat "incorrect" — FeedbackBar already distinguishes
 * correct/review tones, so this body only adds the model reference answer
 * when the server reveals one (PRACTICE mode).
 */
export function ShortAnswerBody({ display, disabled, verdict, onAnswerChange }: ExerciseBodyProps) {
  const { t } = useTranslation();
  const { colors } = useTheme();
  const styles = makeStyles(colors);
  const content = display.content as unknown as ShortAnswerContent;
