import { listeningRepository } from './ListeningRepository';
...
describe('listeningRepository.getQuestionsForDeck', () => {
  it('asks about every word of a deck smaller than the option count', async () => {
    ...
    const questions = await listeningRepository.getQuestionsForDeck(1, 'ru');
  ...
  it('tops a small deck up from the rest of the dictionary', async () => {
    ...
    const [question] = await listeningRepository.getQuestionsForDeck(1, 'ru');
  ...
  it('does the same for an overrideWordIds subset below the option count', async () => {
    ...
    const questions = await listeningRepository.getQuestionsForDeck(1, 'ru', [1, 2]);
  ...
  it('drops a word the dictionary cannot distract from at all', async () => {
    ...
    expect(await listeningRepository.getQuestionsForDeck(1, 'ru')).toEqual([]);
  });
